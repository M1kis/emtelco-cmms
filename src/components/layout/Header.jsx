    ...
    <header className="sticky top-0 z-30 bg-white/85 backdrop-blur-md border-b border-zinc-200/80 px-4 lg:px-8 py-2.5 transition-all">
      <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
        ...
        <button
          ...
          className="hidden md:flex items-center gap-3 px-3 py-1.5 rounded-md border border-zinc-200 hover:border-zinc-300 bg-zinc-50/70 hover:bg-zinc-100 text-zinc-400 text-xs transition-all w-64 justify-between cursor-pointer"
        >
          <div className="flex items-center gap-2">
            <Search className="w-3.5 h-3.5 text-zinc-400" />
            <span className="text-zinc-500 font-normal">Buscar equipos o acciones...</span>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* User selector (Sin imagen de perfil, solo inicial / nombre) */}
          <div className="relative">
            <button
              onClick={() => setUserDropdownOpen(!userDropdownOpen)}
              className="btn-interactive flex items-center gap-2 px-2.5 py-1 rounded-md hover:bg-zinc-100/80 border border-zinc-200 hover:border-zinc-300 transition-all text-left shadow-2xs cursor-pointer bg-white"
            >
              <div className="w-5 h-5 rounded bg-zinc-100 text-zinc-800 flex items-center justify-center font-bold text-[10px] font-mono border border-zinc-200">
                ...
                <div className="py-1">
                  {availableUsers.map((user) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 flex items-center justify-between text-xs transition-colors duration-100 cursor-pointer ${
