...
export const Header = ({ onOpenMobileMenu, currentView, onOpenSupabaseModal }) => {
  const { currentUser, switchUser, availableUsers } = useAuth();
  const { isSupabaseActive } = useData();
  const [userDropdownOpen, setUserDropdownOpen] = useState(false);

  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-zinc-200/80 px-4 lg:px-8 py-2.5 transition-all">
      <div className="flex items-center justify-between gap-4 max-w-7xl mx-auto">
        
        {/* Izquierda: Botón Menú Mobile + Breadcrumb */}
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-xs select-none">
            <span className="text-zinc-400 font-medium hidden sm:inline transition-colors hover:text-zinc-600">EMTELCO</span>
            <span className="text-zinc-300 hidden sm:inline">/</span>
            <span className="text-zinc-400 font-medium hidden sm:inline transition-colors hover:text-zinc-600">TICS</span>
            <span className="text-zinc-300 hidden sm:inline">/</span>
            <span className="font-semibold text-zinc-900 tracking-tight">{currentView}</span>
          </div>
        </div>

        {/* Derecha: Estado DB & Selector de Usuario */}
        <div className="flex items-center gap-2.5">
          
          {/* Status Database */}
          <button
            onClick={onOpenSupabaseModal}
            className="btn-interactive flex items-center gap-2 px-2.5 py-1 rounded-md text-[11px] font-medium border border-zinc-200 hover:border-zinc-300 hover:bg-zinc-50 transition-all text-zinc-700 bg-white shadow-2xs group"
            title="Estado de conexión Supabase"
          >
            <span className={`w-1.5 h-1.5 rounded-full transition-transform duration-200 group-hover:scale-125 ${isSupabaseActive ? 'bg-emerald-500 shadow-xs' : 'bg-amber-500'}`} />
          ...
          <div className="relative">
            ...
            >
              <img 
                ...
                className="w-6 h-6 rounded-full object-cover grayscale contrast-125 transition-transform duration-200 hover:scale-105"
              />
